...
class String {
   ...
   // $a=str_shuffle("123456");    随机打乱字符串                 
                $a="ABCDEFGH|JK£MNOP@RSTUVWXYZ123456789";
                
                
$b= str_shuffle($a);
              
     //  str_shuffle($a);随机打乱字符串顺序         
 ...
 * *@$chars='ABCDEFGHIJKMNPQRSTUVWXYZ123456789'.$addChars;
    ...
    static public function autoCharset($string, $from='gbk', $to='utf-8') {
        ...
        if (e($from) === strtoupper($to) || empty($string) || (is_scalar($string) && !is_string($string))) {
            //如果编码相同或者非字符串标量则不转换
            return $string;
        }
        if (is_string($string)) {
